...
| FR-007 | Reps must be a whole number of at least 1. Weight must be zero or more and accept fractional plates. An invalid entry explains which field is wrong and records nothing. | `0` reps and `-5` kg are rejected with a message on the field; `2.5` kg is accepted.                            |
...
- **The same exercise twice in one day.** He returns to a machine later in the
  session. Whether that is one entry or two depends on how a session is bounded —
  see the open questions.
- **Identical sets.** 60 × 10 three times in a row is normal and must not be
  deduplicated (FR-006).
- **Midnight.** A set at 23:58 and the next at 00:03 belong to the same training
  session as far as he is concerned, but not to the same date.
...
- **A very long or emoji-laden exercise name.** Must not break the list or the
  search.
- **First ever launch.** No exercises, no history: the app must be usable rather
  than presenting an empty screen with nothing to do (relates to FR-004).
...
- **The data lives on the device.** What that implies for a replaced or lost
  phone is an open question below, not a settled decision.

## Open questions

- [ ] `[NEEDS CLARIFICATION: How is "the same exercise" identified across sessions — picked from a list the app maintains, or typed freely each time? Free typing fragments history on any spelling variation, which would defeat FR-003; a list needs a starting set of exercises from somewhere.]`
- [ ] `[NEEDS CLARIFICATION: You go to whichever machine is free, and 60 kg on one leg press is not 60 kg on another — plate stacks and leverage differ between machines and gyms. Should history be scoped to a specific machine, or is exercise-level history close enough to be useful?]`
- [ ] `[NEEDS CLARIFICATION: What exactly does "last time" need to show to build a progression — only the previous session's sets, the last few sessions, or the best set ever recorded?]`
- [ ] `[NEEDS CLARIFICATION: Is a workout an explicit thing you start and finish, or simply "the sets recorded on a given day"? This decides the midnight edge case and whether returning to a machine later counts as one entry or two.]`
- [ ] `[NEEDS CLARIFICATION: Is weight always a single number in one unit? Pull-ups are bodyweight, assisted machines subtract weight, dumbbells are usually written per hand, and some machines are marked in plates rather than kilograms.]`
- [ ] `[NEEDS CLARIFICATION: Mistyping mid-set is inevitable. Does the first version need to edit or delete a recorded set, or is living with a wrong number acceptable until later?]`
- [ ] `[NEEDS CLARIFICATION: With data only on the device, a lost or replaced phone loses every record. Does the first version need any way to get the data out, or is that a risk you accept for now?]`
